...
type TokenIconMeta = { iconUrl?: string }
...
// only treat as LP if token0 and token1 are real addresses
if (
  !token0Addr ||
  !token1Addr ||
...
function guessVaultFromToken(t: TokenHolding): { protocol: string; pool: string } | null {
  ...
  // everything else is not a pool, so ignore it here
  return null
}
...
export async function GET(req: Request) {
  try {
    ...
    const [tokens, nadoUsd] = await Promise.all([fetchErc20Tokens(wallet), fetchNadoUsdEquity(wallet)])

    const stableSymbols = new Set(['USDC', 'USDT', 'DAI', 'GHO', 'FRAX', 'SUSD'])

    const addrList = Array.from(
      new Set(tokens.map((t) => t.address).filter((addr) => addr && addr !== 'native'))
    ).slice(0, 20)
...
return { ...v, factoryAddress: v.factoryAddress || null, creatorAddress: creator || v.creatorAddress || null }
    ...
    console.error('api/positions fatal error', err)
    return NextResponse.json({ ok: false, error: 'internal positions error' }, { status: 500 })
  }
}
